...
export class AIController {
  ...
  async getFullContext(): Promise<AIContext> {
    try {
      ...
      try {
        const scansResult = await this.db.query(
          `SELECT scan_id, system_info, exploits_found, analysis_status, created_at 
      ...
      return {
        ...
        siteState: {
          ...
          pendingScans: vulnerabilityScans.filter((s: any) => s.analysis_status === 'pending').length
        }
      };
    } catch (error) {
  ...
  async analyzeVulnerabilityScan(scanId: string): Promise<any> {
    try {
      // Get scan data
      const result = await this.db.query(
        'SELECT * FROM vulnerability_scans WHERE scan_id = $1',
        [scanId]
      );
      ...
      await this.db.query(
        `UPDATE vulnerability_scans 
         ...
         WHERE scan_id = $3`,
        [JSON.stringify(exploits), 'completed', scanId]
      );
  ...
  async findExploits(systemInfo: any): Promise<any[]> {
    const exploits: any[] = [];
